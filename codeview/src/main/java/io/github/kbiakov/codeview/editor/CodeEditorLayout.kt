...
constructor(
    ...
    var showDivider: Boolean
        ...
        get() = dividerView.visibility == VISIBLE
    ...
    var showMinimap = DEFAULT_SHOW_MINIMAP
        set(value) {
            field = value
            minimapContainerLayout.visibility = if (value) VISIBLE else GONE
    ...
    private fun readParameters(attrs: AttributeSet?, defStyleAttr: Int) {
        context.withStyledAttributes(attrs, R.styleable.CodeEditorLayout, defStyleAttr, 0) {
            ...
            val lineNumberTextColor = getColor(
                context,
            ...
            val lineNumberBackgroundColor = getColor(
                context,
            ...
            showDivider =
                getBoolean(R.styleable.CodeEditorLayout_ke_divider_enabled, DEFAULT_SHOW_DIVIDER)
            ...
            editorBackgroundColor = getColor(
                context,
                defaultColor = Color.WHITE,
            ...
            isMoveWithCursorEnabled =
                getBoolean(R.styleable.CodeEditorLayout_ke_editor_followCursor, true)

            val codeEditorMaxZoom = getFloat(
                R.styleable.CodeEditorLayout_ke_editor_maxZoom,
                CodeEditorView.Companion.DEFAULT_MAX_ZOOM
            )
            ...
            showMinimap =
                getBoolean(R.styleable.CodeEditorLayout_ke_minimap_enabled, DEFAULT_SHOW_MINIMAP)
            minimapMaxDimension = getDimensionPixelSize(
                R.styleable.CodeEditorLayout_ke_minimap_maxDimension,
                DEFAULT_MINIMAP_MAX_DIMENSION_DP
            ).toFloat()
            minimapBorderColor = getColor(
                context,
                defaultColor = Color.BLACK,
            ...
            minimapIndicatorColor = getColor(
                context,
        ...
        codeEditorView.selectionChangedListener = object : SelectionChangedListener {
            override fun onSelectionChanged(start: Int, end: Int, hasSelection: Boolean) {
                ...
                try {
                    ...
                    Log.e(CodeEditorView.Companion.TAG, "Error moving screen with cursor", e)
                }
            }
        }
        ...
        codeEditorView.codeEditText.textChanges()
            ...
            .onEach {
                try {
                    ...
                    Log.e(CodeEditorView.Companion.TAG, "Error updating line numbers", e)
                }
            }
            .catch {
                Log.e(
                    CodeEditorView.Companion.TAG,
                    "Unrecoverable error while updating line numbers",
                    it
                )
    ...
    private fun moveEditorToPercentage(percentageX: Float, percentageY: Float) {
        val targetX =
            -codeEditorView.engine.computeHorizontalScrollRange() / codeEditorView.engine.zoom * percentageX
        val targetY =
            -codeEditorView.engine.computeVerticalScrollRange() / codeEditorView.engine.zoom * percentageY
    ...
    private fun updateMinimapImage() {
        ...
        val targetView: View =
            if (editable) codeEditorView.codeEditText else codeEditorView.codeTextView
        targetView.apply {
            post {
                createSnapshot(
    ...
    private fun updateMinimapIndicator(editorRect: Rect = calculateVisibleCodeArea()) {
        ...
        (minimapIndicator.layoutParams as MarginLayoutParams).apply {
            topMargin = ((minimapZoomLayout.height *
                    (engine.computeVerticalScrollOffset()
                        .toFloat() / engine.computeVerticalScrollRange()))).roundToInt()
            leftMargin = ((minimapZoomLayout.width *
                    (engine.computeHorizontalScrollOffset()
                        .toFloat() / engine.computeHorizontalScrollRange()))).roundToInt()

            width = (minimapZoomLayout.width * (editorRect.width()
                .toFloat() / engine.computeHorizontalScrollRange())).roundToInt()
            height = (minimapZoomLayout.height * (editorRect.height()
                .toFloat() / engine.computeVerticalScrollRange())).roundToInt()
    ...
    private fun updateLineNumbers(
        editorRect: Rect = calculateVisibleCodeArea(),
    ...
    private fun getCursorScreenPosition(): PointF? {
        ...
        return PointF(
            (x + codeEditorView.panX) * codeEditorView.realZoom,
