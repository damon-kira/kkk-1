...
@JvmOverloads constructor(context: Context, attrs: AttributeSet? = null, defStyleAttr: Int = 0) :
    ...
    var languageRuleBook: LanguageRuleBook?
        ...
        set(value) {
            if (value != null) {
                ...
                codeTextView.highlighter =
                    StatefulSpannableHighlighter(value, value.defaultColorScheme)
            } else {
    ...
    var editable: Boolean
        get() = codeEditText.isVisible
        set(value) {
            if (value) {
                codeEditText.visibility = VISIBLE
                codeTextView.visibility = GONE
            } else {
                codeTextView.text = codeEditText.text
                codeEditText.visibility = GONE
    ...
    private fun readParameters(attrs: AttributeSet?, defStyleAttr: Int) {
        context.withStyledAttributes(attrs, R.styleable.CodeEditorView, defStyleAttr, 0) {
            ...
            val maxRealZoom =
                getFloat(R.styleable.CodeEditorView_ke_editor_maxZoom, DEFAULT_MAX_ZOOM)
