...
class DialogManager {
    ...
    fun show(activity: Activity): AlertDialog? {
        ...
        activity.showDialoga(alertDialog)
        alertDialog?.getButton(AlertDialog.BUTTON_NEGATIVE)?.setTextColor(activity.resources.getColor(
            R.color.dialog_text))
        alertDialog?.getButton(AlertDialog.BUTTON_POSITIVE)?.setTextColor(activity.resources.getColor(R.color.dialog_title_text_color_blue))
        return alertDialog
    }
