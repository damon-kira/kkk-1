 ...
 * 单文件上传
 */
class QuickUploadSingleActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        uploadStart.setOnClickListener {
            if (fileItem == null) {
                ...
                fileItem?.let {
                    it.quickUploadRequest= QuickUploadRequest(this, serverUrl = "http://www.kira-learning.com:8080/upload")
                        .setMethod("POST")
                        .addFileToUpload(
    ...
    private fun onPickedFiles(path: String) {
        fileItem = FileItem(name(path.toUri()), path, path)
        fileItem?.refresh { uploadStatus, uploadInfo, throwable, serverResponse ->
            when (uploadStatus) {
                UploadStatus.DEFAULT -> {
                    uploadProgress.text = "默认"
                }

                UploadStatus.Wait -> {
                    uploadProgress.text = "等待上传"
                }
                ...
                UploadStatus.Error -> {
                    uploadProgress.text = "错误：${throwable.toString()}"
