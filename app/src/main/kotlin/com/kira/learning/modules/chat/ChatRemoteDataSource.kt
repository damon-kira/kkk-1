...
class RealChatRemoteDataSource @Inject constructor(private val api: ComposeApiService) :
    ...
    override suspend fun sendOnce(prompt: String): ApiResult<String> = runCatching {
        ...
        onFailure = {
            ApiResult.Error(
                code = -1,
                message = it.message ?: "网络错误",
                errorType = com.kira.learning.network.ErrorType.UNKNOWN,
                throwable = it
            )
        }
    )
    ...
    override fun stream(prompt: String): Flow<ApiResult<String>> = flow {
        // 真实后端暂无 SSE/WebSocket，取完整回答后按固定长度切片模拟流式体
...
private fun <T> ApiResult<T>.mapData(): ApiResult<String> = when (this) {
    ...
    is ApiResult.Error -> ApiResult.Error(code, message, errorType, throwable)
    is ApiResult.NetworkUnavailable -> ApiResult.NetworkUnavailable
    is ApiResult.Loading -> ApiResult.Loading(message)
}
